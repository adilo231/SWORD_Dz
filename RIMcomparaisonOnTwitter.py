...
def HISBmodel (Graph,Seed_Set,Opinion_Set,Statistical,paramater,K,Tdet,method):
    ...
    #      update_links(Graph, stubs, percentage_adding) 
      ...
      if time >=Tdet*0.125 and bl<K  and method != 'NP' :
          ...
          if (method=='AGPTCS'):
              mvc_BetaD_tcs_temprary_tolerance_adaptative(Graph,p)
              Liste_protector=Protector(Graph)
              taille=len(Liste_protector)
             # print(taille)
              for i in range(taille):
...
def graphe_TO_json(g):
    ...
    return data
def geneList_Infectede(g,Listinfected,Listopinion,N,percentage):
...
def Start(i,Graph,parameter,Stat,percentage,K,Tdet,method):
    ...
    geneList_Infectede(Graph,ListInfected,Listopinion,len(Graph.nodes),percentage)
...
def Display(Stat_Global,xx,title_fig,nb):
   #print(Stat_Global)
    ...
    Title=['NP','TCSCen','BNLSCen','BLS','AGPUX']
    max=0
    Stat=[]
    Infected=[]
    para=[]
...
def DRIMUX(G, k, t):
    ...
    for key in ranked_listDRIMUX:
        if counter > k_i:
            break
        G.nodes[key[0]]['Blockingtime']= 43.74*49/G.nodes[key[0]]['degree']
...
def mvc_BetaD_Blocking_nodes_temprary_tolerance_adaptative(G,k):
    ...
    for i in range(k_i):
            ID = betaD.index(max(betaD))
            if G.nodes[nb[ID]]['BlockedTemporary'] != 'true':
                G.nodes[nb[ID]]['Blockingtime']=3687*48/G.nodes[nb[ID]]['degree'] # *(50-1)==time of the whole process/ (maxDeg-minDeg)==(1045-1)
                G.nodes[nb[ID]]['BlockedTemporary']='true'
            betaD.pop(ID)
            nb.pop(ID)   
def mvc_BetaD_tcs_temprary_tolerance_adaptative(G,k):
    sp=[]
    ...
    for i in range(k_i):
            ID = beta2D.index(max(beta2D))
            G.nodes[nb[ID]]['Protector']='True'
            G.nodes[nb[ID]]['state']='infected'
            beta2D.pop(ID)
            nb.pop(ID)   
def mvc_BetaD_Blocking_nodes_temprary_tolerance1(G,Nbr_Infected,k,ratio,k0):
    ...
    for i in range(k_i):
            if len(betaD) == 0:
                break
            ID = betaD.index(max(betaD))
            G.nodes[nb[ID]]['Blockingtime']=G.nodes[nb[ID]]['degree']*98/3687 # *(50-1)==time of the whole process/ (maxDeg-minDeg)==(1045-1)
            G.nodes[nb[ID]]['BlockedTemporary']='true'
            betaD.pop(ID)
            nb.pop(ID)   
...
# change float to int.
    ...
    g=json_graph.node_link_graph(geneTwitter.txt2Graph('twitter.txt'))
    ...
    Nodes=len(g.nodes)
    static="Nodes :{},Edegs:{}."
    percentage=0.01 #1% of popularity" is infected 
    NumOFsumi=1
    beta=0.2
    omega=0 
    juge=0.1
    ...
    for Tdetection in [1, 5, 10, 15]:
        for k_ratio in [0.01, 0.05, 0.10, 0.15, 0.20]:
            K=int(Nodes*k_ratio)
