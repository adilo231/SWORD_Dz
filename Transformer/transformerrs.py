...
class transform :
    def __init__(self,collection_names,db_name,mongo_client_url):
        self.collection_names=collection_names
        self.db_name=db_name
        self.mongo_client_url=mongo_client_url

    def remove_and_update_null(self,verbose=False):
        client = pymongo.MongoClient(self.mongo_client_url)

        # Select the database and collection
        db = client[self.db_name]
        for  collection_name in tqdm(self.collection_names,desc='Remove Null Attributs'):
    ...
    def doc_update_tokens(self):
        client = pymongo.MongoClient(self.mongo_client_url)

        # Select the database and collection
        db = client[self.db_name]
        for  collection_name in tqdm(self.collection_names,desc='Tokenisation'):
    ...
    def string_to_datetime(self):
        ...
        # Select the database and collection
        db = client[self.db_name]
        for  collection_name in tqdm(self.collection_names,desc='String to DateTime'):
            collection=db[str(collection_name)]
            docs=collection.find({})
            for doc in docs:
    ...
    def cloud_of_words(self, verbose=False):
        ...
        for collection_name in tqdm(self.collection_names,desc='Cloud of words'):
            #if verbose:
            ...
            if(len(word_ar)>1):
                ...
                if(i>1):
                    ...
                    plt.title(f'Collection Cloud of word {collection_name} in Arabic')
                    plt.axis('off')
                j=j+1
            ...
            if(len(word_en)>1):
                ...
                if(i>1):
                    ...
                    axs[j].set_title(f'Collection Cloud of word {collection_name} in English')
                else:
                    plt.imshow(wordcloud_en)
                    plt.title(f'Collection Cloud of word {collection_name} in English')
                    plt.axis('off')
    ...
    def tweets_lang_repartition(self):
        # Connect to the MongoDB server
        Stat_Global = pd.DataFrame()
        client = MongoClient(self.mongo_client_url)

        # Select the database and collection
        db = client[self.db_name]

        num_collections = len(self.collection_names)
        cols = int(num_collections / 2) + num_collections % 2
        rows= 2 if num_collections > 1 else 1

        fig, axs = plt.subplots(nrows=rows, ncols=cols, figsize=(10, 6 * rows), squeeze=False)

        i=0
        for collection_name in tqdm(self.collection_names,desc="language distribution"):
    ...
    def tweets_date_repartition(self):
        date_format = '%a %b %d %H:%M:%S %z %Y'
        client = MongoClient(self.mongo_client_url)
        ...
        i=0
        for collection_name in tqdm(self.collection_names,desc='Date Repartition'):
            # Select the collection
            tweets = db[collection_name]
